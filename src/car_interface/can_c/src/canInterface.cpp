...
{   
    int32_t val = ((static_cast<uint32_t>(msg[4]) << 24) |
               (static_cast<uint32_t>(msg[3]) << 16) |
               (static_cast<uint32_t>(msg[2]) << 8)  |
               static_cast<uint32_t>(msg[1]));

    if (rl_wheel_speed_cycles == 0){
        rl_wheel_speed_cycles = val;
    }else if (rl_wheel_speed_cycles != val){
        moving = true;
    }

    float speed = 0;
    if (moving){
        speed = (72000000.0f / static_cast<float>(val)) * 2 * M_PI * 0.2f / 12;
    }
