...
class Controller():
    ...
    def set_mpc_route(self, msg:Trajectory):
        if self.FIRST_LAP:
            ...
            curva2 = Marker()
            curva2.header.frame_id = "map"
            curva2.ns = 'mpc'
            curva2.id = 35
            curva2.action = Marker.ADD
            curva2.type = Marker.LINE_STRIP
            curva2.points = [Point(p[0],p[1],0) for p in route]
            curva2.color.g = 1
            curva2.color.a = 1
            curva2.scale.x = 0.1
            curva2.pose.orientation.x = 0.0
            curva2.pose.orientation.y = 0.0
            curva2.pose.orientation.z = 0.0
            curva2.pose.orientation.w = 1.0
            curvas.markers.append(curva2)
    ...
    def send_controllers_pp(self, msg):
        if self.FIRST_LAP:
            ...
            self.par = msg.accelerator
            self.delta = math.radians(msg.steering)
