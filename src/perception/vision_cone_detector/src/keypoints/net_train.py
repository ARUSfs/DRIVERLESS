...
def train_model(kNet, epochs, dataset_images_path,
    ...
    parameters to do it.
    '''
    criterion = CrossRatioLoss(loss_type, include_geo,
                               geo_loss_gamma_horz,
                               geo_loss_gamma_vert)
    ...
    fit(kNet, dataloader, epochs, criterion, scheduler,
        optimizer, num_kpt, device)


def fit(net, dataloader, epochs, loss_function, scheduler,
        optimizer, num_kpt, device):
    ...
    for epoch in range(1, epochs+1):
        ...
        for X, Y_hm, Y_points, name, imsize in train_process:
            ...
            train_process.set_description(f"Batch {batch_num}. Location Loss: {round(loc_loss,5)}. \
                                          Geo Loss: {round(geo_loss,5)}. \
                                          Total Loss: {round(loss,5)}")
        ...
        print(f"\tTraining: MSE/Geometric/Total Loss: \
              {round(total_loss[0]/batch_num,10)}/ \
              {round(total_loss[1]/batch_num,10)}/ \
              {round(total_loss[2]/batch_num,10)}")
        ...
        print(f"\tValidation: MSE/Geometric/Total Loss: \
              {round(val_loc_loss,10)}/ {round(val_geo_loss,10)}/ {round(val_loss,10)}")
        ...
        if tolerance >= max_tolerance:
            print(f"Training is stopped due; loss no longer decreases. \
                  Epoch {best_epoch} is has the best validation loss.")
            break
